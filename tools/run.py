...
modeld.load_state_dict(torch.load(('./ckpt/' + myname + '1' + '.ckpt'), map_location=(torch.device('cpu'))))
outputd = modeld(f)
...
for x in range(len(pred_d)):
    ...
    modelr.load_state_dict(torch.load(('./ckpt/' + myname + '2' + '.ckpt'), map_location=(torch.device('cpu'))))
    outputr = modeld(f)
    ...
    for x in range(len(pred_r)):
        ...
        modelp.load_state_dict(torch.load(('./ckpt/' + myname + '3' + '.ckpt'), map_location=(torch.device('cpu'))))
        outputp = modeld(f)
        ...
        for x in range(len(pred_p)):
            ...
            modelc.load_state_dict(torch.load(('./ckpt/' + myname + '4' + '.ckpt'), map_location=(torch.device('cpu'))))
            outputc = modeld(f)
